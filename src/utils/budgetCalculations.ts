...
export const CATEGORY_WEIGHTS = {
  food: { clean_weight: 1.30, night_weight: 1.40 },
  bar: { clean_weight: 1.30, night_weight: 1.40 },
  cafe: { clean_weight: 1.20, night_weight: 1.30 },
  restaurant: { clean_weight: 1.30, night_weight: 1.40 },
  entertainment: { clean_weight: 1.20, night_weight: 1.50 },
  retail: { clean_weight: 1.00, night_weight: 1.00 },
  shopping: { clean_weight: 1.00, night_weight: 1.00 },
  service: { clean_weight: 0.95, night_weight: 0.90 },
  services: { clean_weight: 0.95, night_weight: 0.90 },
  lodging: { clean_weight: 1.10, night_weight: 1.20 },
  hotel: { clean_weight: 1.10, night_weight: 1.20 },
  other: { clean_weight: 1.00, night_weight: 1.00 }
};

// Calculate service intensity from business mix
export function calculateServiceIntensity(categoryBreakdown: Record<string, number>) {
  let totalWeightedClean = 0;
  let totalWeightedNight = 0;
  let totalBusinesses = 0;
  
  for (const [category, count] of Object.entries(categoryBreakdown)) {
    const weights = CATEGORY_WEIGHTS[category.toLowerCase()] || CATEGORY_WEIGHTS.other;
  ...
  return {
    cleanIntensity: totalBusinesses > 0 ? totalWeightedClean / totalBusinesses : 1.0,
    nightIntensity: totalBusinesses > 0 ? totalWeightedNight / totalBusinesses : 1.0
  };
}
...
export function calculateCleaningCost(
  ...
  const cleanerHoursPerDay = (frontageEstimate / params.frontage_ft_per_cleaner_hour) * 
                              params.intensity_weight_clean * cleanIntensity;
  
  const cleanersNeeded = Math.ceil(cleanerHoursPerDay / params.clean_hours_per_shift);
  const supervisorsNeeded = Math.ceil(cleanersNeeded / params.supervisor_ratio);
  
  const cleanerCost = cleanersNeeded * params.clean_hours_per_shift * 
                      params.clean_days_per_week * 52 * params.clean_loaded_rate;
  
  const supervisorCost = supervisorsNeeded * params.clean_hours_per_shift * 
                         params.clean_days_per_week * 52 * params.supervisor_loaded_rate;
...
export function calculateSafetyCost(
  ...
  const adjustedFTE = baselineFTE * nightIntensity * params.intensity_weight_safety;
  
  return adjustedFTE * 8 * params.safety_days_per_week * 52 * params.safety_loaded_rate;
}
...
export function calculateAssetsCost(
  ...
  const trashCanCost = (trashCans * params.trash_can_unit_cost) / params.trash_can_life_years;
  const planterCost = (planters * params.planter_unit_cost) / params.planter_life_years;
  const bannerCost = (banners * params.banner_unit_cost) / params.banner_life_years;
...
export function calculateMarketingCost(
  ...
  const nightEconomyBonus = params.marketing_night_economy_multiplier * 
                            nightIntensity * params.marketing_base_annual * 0.15;
...
export function calculateBudget(
  ...
  const { cleanIntensity, nightIntensity } = calculateServiceIntensity(categoryBreakdown);
  
  // Calculate individual service costs
  const cleaningCost = calculateCleaningCost(params, frontageEstimate, cleanIntensity);
  const safetyCost = calculateSafetyCost(params, nightIntensity);
  const assetsCost = calculateAssetsCost(params, frontageEstimate);
  const marketingCost = calculateMarketingCost(params, businessCount, nightIntensity);
  ...
  return {
    ...
    // Staffing estimates
    cleanersNeeded: Math.ceil((frontageEstimate / params.frontage_ft_per_cleaner_hour) * 
                              params.intensity_weight_clean * cleanIntensity / 
                              params.clean_hours_per_shift),
    supervisorsNeeded: Math.ceil(Math.ceil((frontageEstimate / params.frontage_ft_per_cleaner_hour) * 
                                           params.intensity_weight_clean * cleanIntensity / 
                                           params.clean_hours_per_shift) / params.supervisor_ratio),
    safetyFTE: params.safety_enabled ? 
               Math.ceil(params.safety_hours_per_day / 8.0) * nightIntensity : 0
  };
}

// Determine place typology based on business mix
export function determinePlaceTypology(categoryBreakdown: Record<string, number>, totalPlaces: number) {
  ...
  if (topCategory.percentage > 40) {
    ...
    if (topCategory.category === 'retail' || topCategory.category === 'shopping') {
      return 'Retail Core';
    } else if (topCategory.category === 'food' || topCategory.category === 'restaurant') {
      return 'Dining District';
    } else if (topCategory.category === 'service' || topCategory.category === 'services') {
      return 'Service Hub';
  ...
  const foodPercentage = (categoryBreakdown.food || 0) + (categoryBreakdown.restaurant || 0);
  const retailPercentage = (categoryBreakdown.retail || 0) + (categoryBreakdown.shopping || 0);
...
export function getServiceDemandIndicators(
  ...
  const foodPercentage = ((categoryBreakdown.food || 0) + (categoryBreakdown.restaurant || 0)) / businessCount * 100;
  const retailPercentage = ((categoryBreakdown.retail || 0) + (categoryBreakdown.shopping || 0)) / businessCount * 100;
  const entertainmentPercentage = (categoryBreakdown.entertainment || 0) / businessCount * 100;
  
  return {
    cleaning: {
      priority: cleanIntensity > 1.15 ? 'High' : cleanIntensity > 1.05 ? 'Medium' : 'Standard',
      needs: [
    ...
    safety: {
      priority: nightIntensity > 1.2 ? 'High' : nightIntensity > 1.1 ? 'Medium' : 'Standard',
      needs: [
    ...
    marketing: {
      priority: businessCount > 75 ? 'High' : businessCount > 35 ? 'Medium' : 'Standard',
      needs: [
